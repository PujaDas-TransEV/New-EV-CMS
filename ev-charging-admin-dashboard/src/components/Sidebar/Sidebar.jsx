...
/* ------------------------------------------------------------------
   ...
   ------------------------------------------------------------------
   The Sidebar is re-mounted by every page. Without this cache, each
   navigation would refetch the org and flash "Loading..." plus hide
   the admin-only menu items until the role arrives, causing the last
   few items to visibly shift/blink. Caching makes remounts instant.
   ------------------------------------------------------------------ */
...
const Sidebar = ({ isDarkMode = false, onThemeToggle }) => {
  ...
  /* Derive identity directly from the context — no refetch, no flash.
     AuthContext bootstraps /auth/me once, before any page (and thus
     any Sidebar) is mounted, so these values are stable from the
     very first render. */
  ...
  /* ------------------------------------------------------------------
     Menu items
     ------------------------------------------------------------------
     Memoized so the admin-only gate doesn't rebuild on every render.
     Because `userRole` is already populated, admin items are correct
     on the first paint — no appearance/disappearance blink.
     ------------------------------------------------------------------ */
  ...
  useEffect(() => {
    ...
    // Another mount is already fetching — piggyback on it.
    if (orgFetchPromise) {
      let cancelled = false;
      orgFetchPromise
        .then((data) => {
    ...
    orgFetchPromise = authenticatedRequest(API_CONFIG.ORGANIZATION_API, { method: 'GET' })
      ...
      .then((data) => {
        if (data && typeof data === 'object') {
          cachedOrgData = data;               // cache successful result
          if (!cancelled) setOrgData(data);
        } else if (!cancelled) {
          setOrgData({ business_name: 'TransEV' }); // display fallback, don't cache
        }
        return data;
      })
      .catch((error) => {
  ...
  /* ------------------------------------------------------------------
     Logout
     ------------------------------------------------------------------
     AuthContext owns token cleanup and server-side logout.
     ------------------------------------------------------------------ */
  const handleLogout = async () => {
    if (isLoggingOut || isRefreshing) return;
    ...
    try {
      ...
      console.error('Logout error:', error);
      // AuthContext.logout() already clears the session in its finally
      // block, so we don't touch localStorage here.
    } finally {
      setIsLoggingOut(false);
    }
  };
